...
export function useMiniMaxRecordings() {
  const [recordings, setRecordings] = useState<MiniMaxRecording[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const loadRecordings = useCallback(async () => {
    try {
      const res = await fetch('/api/minimax/recordings');
  ...
  const deleteRecording = useCallback(async (id: string) => {
    const res = await fetch(`/api/minimax/recordings/${id}`, { method: 'DELETE' });

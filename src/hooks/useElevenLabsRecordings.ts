...
export function useElevenLabsRecordings() {
  const [recordings, setRecordings] = useState<ElevenLabsRecording[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);

  const loadRecordings = useCallback(async () => {
    try {
      const res = await fetch('/api/elevenlabs/recordings');
  ...
  const deleteRecording = useCallback(async (id: string) => {
    const res = await fetch(`/api/elevenlabs/recordings/${id}`, { method: 'DELETE' });
